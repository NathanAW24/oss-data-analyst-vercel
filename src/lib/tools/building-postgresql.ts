// Building phase tools for SQL generation and validation (PostgreSQL)
...
export const JoinPathFinder = tool({
  description:
    "Compute minimal join path & alias map from selected entities in plan for a PostgreSQL database.",
...
export const BuildSQL = tool({
  description:
    "Generate simple PostgreSQL SQL from the query request. Provide dimensions (columns to select/group by), measures (aggregations), filters, orderBy, and limit.",
  inputSchema: z.object({
    baseEntity: z
      .string()
      .min(1)
      .describe("Primary entity (e.g., 'Accounts', 'People', 'Company')"),
    dimensions: z
      .array(z.string())
      .default([])
      .describe(
        "Columns to select and group by (e.g., 'account_number', 'status')"
      ),
    measures: z
      .array(z.string())
      .default([])
      .describe("Aggregations to compute (e.g., 'SUM(monthly_value)', 'COUNT(*)')"),
    filters: z
      .array(z.string())
      .default([])
      .describe("WHERE conditions (e.g., \"status = 'Active'\")"),
    orderBy: z
      .array(z.string())
      .default([])
      .describe("ORDER BY clauses (e.g., 'monthly_value DESC')"),
      ...
      .array(
        z.object({
          ...
          on: z
            .string()
            .describe(
              "Join condition (e.g., 'accounts.company_id = companies.id')"
            ),
        })
      )
      .default([])
      .describe("Tables to join"),
  }),
  execute: async ({
    baseEntity,
    dimensions,
    ...
    console.log("[BuildSQL] Building simple PostgreSQL query");
...
export const SyntaxValidator = tool({
  description:
    "Static scan of SQL to block multi-statements, DDL/DML, and risky constructs. Validates PostgreSQL compatibility.",
...
export const SemanticValidator = tool({
  description:
    "Validate that referenced fields/metrics exist and joins/time-grain make sense for PostgreSQL tables.",
...
export const ValidateSQL = tool({
  description:
    "Check the SQL for syntax safety and semantic correctness against the plan. Ensures PostgreSQL compatibility.",
  inputSchema: z.object({
  ...
  execute: async ({ plan, sql }) => {
    ...
    if (syntaxOk && semanticOk && notes.length === 0) {
      notes.push("All validation checks passed - SQL is PostgreSQL compatible");
    }
...
export const FinalizeBuild = tool({
  description: "Lock the SQL and validation summary for execution against a PostgreSQL database.",
  inputSchema: z.object({
    sql: z.string().min(1),
    validation: z.object({
      syntaxOk: z.boolean(),
  ...
  execute: async (payload) => {
    console.log("[FinalizeBuild] Finalizing SQL for PostgreSQL execution");
